...
const LG: React.FC<LightGalleryProps> = ({
  ...
  const registerEvents = React.useCallback(() => {
    ...
    if (onRotateLeft && $lg && $lg.current) {
      $lg.current.addEventListener(LgMethods.onRotateLeft, ((event: CustomEvent) => {
        onRotateLeft(event.detail);
      }) as EventListener);
    }
    if (onRotateRight && $lg && $lg.current) {
      $lg.current.addEventListener(LgMethods.onRotateRight, ((event: CustomEvent) => {
        onRotateRight(event.detail);
      }) as EventListener);
    }
    if (onFlipHorizontal && $lg && $lg.current) {
      $lg.current.addEventListener(LgMethods.onFlipHorizontal, ((event: CustomEvent) => {
        onFlipHorizontal(event.detail);
      }) as EventListener);
    }
    if (onFlipVertical && $lg && $lg.current) {
      $lg.current.addEventListener(LgMethods.onFlipVertical, ((event: CustomEvent) => {
  ...
  React.useEffect(() => {
    registerEvents();
    const lightGallery = LightGallery($lg.current as unknown as HTMLElement, restProps);

    return function cleanup() {
      try {
        lightGallery.destroy();
      } catch (e) {
        console.log("LIGHTGALLERY DESTROY ERROR", e);
      }
    };
  }, [registerEvents, restProps]);

  return (
    <div className={`lg-react-element ${elementClassNames ? elementClassNames : ""}`} ref={$lg}>
      {children}
    </div>
  );
};
